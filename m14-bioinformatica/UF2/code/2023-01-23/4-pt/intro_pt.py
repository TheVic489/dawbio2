...
# Constants
DATA_DIR = Path('/bio/2023-01-23/4-pt/data')

CORDYCEPS_JAVANICA_GENBANK       = DATA_DIR/'Cordyceps_javanica.gb'
CORDYCEPS_JAVANICA_ALL_GENBANK   = DATA_DIR/'Cordyceps_javanica_allgenome.gb'
CORDYCEPS_GHANENSIS_GENBANK      = DATA_DIR/'Cordyceps_ghanensis.gb'
CORDYCEPS_CATENIANNULATA_GENBANK = DATA_DIR/'Cordyceps_cateniannulata.gb'
CORDYCEPS_CATENIOBLIQUA_GENBANK  = DATA_DIR/'Cordyceps_cateniobliqua.gb'


def read_genbank(GENBANK_CONST) -> SeqRecord:

    record: SeqRecord = SeqIO.read(GENBANK_CONST,'genbank')

    return record

def get_dict(file):
    dict_thigs = explore(file)
    return dict_thigs

def print_info(record: dict) -> str:

    information = f' Seq: {len(record.seq)}\n Annotations: {len(record.annotations)} \n Features: {len(record.features)}'

    return information

    # SeqRecords can be written to .fasta or .genbank files
    # SeqIO.write(record, fasta_cordyceps_genbank, 'fasta')
    
def get_fasta(record):
    fasta = str(record.seq)
    return fasta

def write_fastas(filename, genbank):  
...
def create_cds(fasta,start,end):
    cds = fasta[start:end+1]
...
# -----------------------------------------------------------------------------
def main():
    record: SeqRecord = SeqIO.read(CORDYCEPS_JAVANICA_ALL_GENBANK,'genbank')
    features = get_features(record.features)
    pp(features)
    fasta = get_seq(record)
    # info = print_info(record)
    # dictionary = get_dict(record)
    # fasta = get_fasta(dictionary)
    # fasta = dictionary.values('seq')
    cds = create_cds(fasta,3217452,3217868)
    print(cds)

    
 
    # anot = get_annotations(record)
    # print(info)
    #pp(anot)
    #pp(f'features : {get_features(record)}')
...
#     record: SeqRecord = SeqIO.read(SARS_COV_2_FASTA, 'fasta')
#     record: SeqRecord = SeqIO.read(CORDYCEPS_JAVANICA_GENBANK, 'genbank')
...
#     genbank_record: SeqRecord = SeqIO.read(CORDYCEPS_JAVANICA_GENBANK, 'genbank')
...
#     record.description        # str         (Genbank's definition line)
...
#     source_location.start          # Position. If fuzzy, can be casted to (int)
#     source_location.end            # Position. If fuzzy, can be casted to (int)
