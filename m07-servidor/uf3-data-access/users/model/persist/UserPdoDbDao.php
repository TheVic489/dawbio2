...
class UserPdoDbDao {
    ...
    public function select(User $entity): ?User {
        ...
        try {
            ...
            $statement  = $connection->prepare($this->queries['SELECT_WHERE_ID']);
            $statement->bindValue("id", $entity->getId(), \PDO::PARAM_INT);
            $success = $statement->execute();
            if ($success) {
                $statement->setFetchMode(\PDO::FETCH_CLASS | \PDO::FETCH_PROPS_LATE, 'user\model\User');
              //$stmt->setFetchMode(\PDO::FETCH_CLASS | \PDO::FETCH_PROPS_LATE, User::class);
                $result = $statement->fetch();  //User|false
                if ($result !== \false) {
                    $data = $result;
                } else {
                    $data = null;
